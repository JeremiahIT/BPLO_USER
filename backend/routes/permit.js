...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const permit_number = `BPLO-${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 9)
        .toUpperCase()}`;
...
router.get("/", async (req, res) => {
  try {
    ...
    // Map file paths → absolute URLs
    const permitsWithUrls = permits.map((p) => {
      const plain = p.get({ plain: true });
      return {
        ...plain,
        dti_certificate: buildFileUrl(req, plain.dti_certificate),
        sec_certificate: buildFileUrl(req, plain.sec_certificate),
        cda_certificate: buildFileUrl(req, plain.cda_certificate),
        bir_certificate: buildFileUrl(req, plain.bir_certificate),
      };
    });

    res.json({ permits: permitsWithUrls });
  } catch (error) {
    console.error("Get permits error:", error);
    res.status(500).json({ error: "Server error", message: error.message });
  }
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const plain = permit.get({ plain: true });
    const permitWithUrls = {
      ...plain,
      dti_certificate: buildFileUrl(req, plain.dti_certificate),
      sec_certificate: buildFileUrl(req, plain.sec_certificate),
      cda_certificate: buildFileUrl(req, plain.cda_certificate),
    ...
    res.json({ permit: permitWithUrls });
  } catch (error) {
    console.error("Get permit error:", error);
    res.status(500).json({ error: "Server error", message: error.message });
  }
