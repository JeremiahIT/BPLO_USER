...
// const { auth, requireRole } = require('../middleware/auth'); // AUTH DISABLED

const router = express.Router();

// Get all permits (AUTH DISABLED)
...
// Get all permits with user info (AUTH DISABLED)
...
// Get single permit by ID (AUTH DISABLED)
...
// Update permit status (AUTH DISABLED)
router.patch('/:id/status', [
  ...
  try {
    ...
    await db.query(
      'UPDATE business_permits SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
      [status, id]
    );
