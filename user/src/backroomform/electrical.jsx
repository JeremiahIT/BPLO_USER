...
export default function ElectricalForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Electrical form submitted successfully!');
      navigate('/');
    } catch (error) {
      console.error(error);
      alert('Error submitting electrical form.');
    }
  };
  ...
  return (
    <div className="electrical-form-container">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Electrical Permit Form</h2>
        <button
          onClick={handleBack}
      ...
      <div className="form-wrapper">
        <form className="form-content" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <label className="form-label">Electrical Clearance Fee:</label>
            <input
              type="text"
              name="electricalClearanceFee"
              placeholder="Enter clearance fee details"
              value={formData.electricalClearanceFee}
              onChange={handleChange}
              className="form-input"
              required
            />
